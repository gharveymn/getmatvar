...
void fillNode(TreeNode *node, int num_chunked_dims)
{
	...
	node->keys = (Key *) malloc((node->num_entries + 1) * sizeof(Key));
	node->children = (TreeNode *) malloc(node->num_entries * sizeof(TreeNode));
	//node->siblings = (TreeNode *)malloc(node->num_entries*sizeof(TreeNode));
	...
	switch (node->node_type)
	{
		case GROUP:
			...
			bytes_needed =
				   8 + 2 * s_block.size_of_offsets + 2 * (node->num_entries + 1) * key_size + 2 * (node->num_entries) * s_block.size_of_offsets;
		...
		case CHUNK:
			...
			node->keys[0].chunk_start = (uint64_t *) malloc((num_chunked_dims + 1) * sizeof(uint64_t *));
			...
			for (int i = 0; i < node->num_entries; i++)
			{
				...
				node->keys[i + 1].chunk_start = (uint64_t *) malloc((num_chunked_dims + 1) * sizeof(uint64_t *));
				for (int j = 0; j < num_chunked_dims + 1; j++)
				{
